...
{
    public class Module_productDAL : Module_newsDAL
    {
        ...
        public override string GetContent(string _moduletype, string _contentid, int _page)
        {
            using (DbOperHandler _doh = new Common().Doh())
            {
                ...
                te.ReplaceContentTag("product", ref PageStr, _contentid);
                ...
                dtContent.Clear();
                dtContent.Dispose();
                return ContentList[0].ToString().Replace("{$Price0}", ContentList[1].ToString()).Replace("{$Price1}", ContentList[2].ToString()).Replace("{$ProductMaxBuyCount}", site.ProductMaxBuyCount + "");
            }
        }
        ...
        {
            string Price0 = JumboECMS.Utils.Strings.ToMoney(dt.Rows[0]["Price0"].ToString());
            string Price1 = JumboECMS.Utils.Strings.ToMoney(dt.Rows[0]["Price1"].ToString());
            ContentList.Add(Price0);
            ContentList.Add(Price1);
        }
    }
}
