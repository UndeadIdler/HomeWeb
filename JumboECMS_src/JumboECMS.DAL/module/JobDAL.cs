...
{
    public class Module_jobDAL : Module_newsDAL
    {
        public Module_jobDAL()
        {
        }
        ...
        public override string GetContent(string _moduletype, string _contentid, int _page)
        {
            using (DbOperHandler _doh = new Common().Doh())
            {
                ...
                te.ReplaceContentTag("job", ref PageStr, _contentid);
                te.ReplaceContentLoopTag(ref PageStr);//主要解决通过tags关联
                ContentList.Add(PageStr);
                p__replaceSingleJob(dtContent, ref PageStr, ref ContentList);
                dtContent.Clear();
                dtContent.Dispose();
        ...
        private void p__replaceSingleJob(DataTable dt, ref string PageStr, ref System.Collections.ArrayList ContentList)
        {
        }
    }
}
