...
{
    public partial class _module_down_edit : JumboECMS.UI.AdminCenter
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            id = Str2Str(q("id"));
            if (id == "0")
                Admin_Load("down-add", "html", "down");
            else
                Admin_Load("down-edit", "html", "down");
            this.txtEditor.Text = AdminName;

            getEditDropDownList(CategoryId, ModuleType, ref ddlReadGroup);

            doh.Reset();
            JumboECMS.DBUtility.WebFormHandler wh = new JumboECMS.DBUtility.WebFormHandler(doh, "jcms_module_down", btnSave);
