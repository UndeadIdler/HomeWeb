...
{
    public partial class category_edit1 : JumboECMS.UI.AdminCenter
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                doh.Reset();
                doh.SqlCmd = "SELECT [ID],[Title],[code] FROM [" + base.CategoryTable + "] WHERE [LanguageCode]='" + _lan + "' AND  len(code)<12 and [TypeId]<2 ORDER BY code";
                DataTable dtCategory = doh.GetDataTable();
                for (int i = 0; i < dtCategory.Rows.Count; i++)
                {
                    ListItem li1 = new ListItem();
                    li1.Value = dtCategory.Rows[i]["Id"].ToString();
                    li1.Text = "├－" + getListName(dtCategory.Rows[i]["Title"].ToString(), dtCategory.Rows[i]["code"].ToString());
                    this.ddlParentId.Items.Add(li1);
                }
                dtCategory.Clear();
                dtCategory.Dispose();
                doh.Reset();
                ...
                doh.Reset();
                doh.SqlCmd = "SELECT [ID],[Title] FROM [jcms_normal_template] WHERE [LanguageCode]='" + _lan + "' AND  sType='subcat' ORDER BY IsDefault desc";
                DataTable dtTemplate = doh.GetDataTable();
                this.ddlTemplateId.Items.Add(new ListItem("==请选择模板==", "0"));
                for (int i = 0; i < dtTemplate.Rows.Count; i++)
                {
                    ListItem li2 = new ListItem();
                    li2.Value = dtTemplate.Rows[i]["Id"].ToString();
                    li2.Text = dtTemplate.Rows[i]["Title"].ToString();
                    this.ddlTemplateId.Items.Add(li2);
                }
                dtTemplate.Clear();
                dtTemplate.Dispose();
            }
        ...
        protected bool chkForm()
        {
            ...
            if (this.ddlTemplateId.SelectedValue == "0" && this.txtAliasPage.Text == "")
            {
                FinalMessage("请在自定义url中输入链接地址", "", 1);
                return false;
            }
            return true;
        }
