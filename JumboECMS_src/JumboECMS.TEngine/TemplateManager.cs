...
namespace JumboECMS.TEngine
{
    ...
    public class TemplateManager
    {
        ...
        void Init()
        {
            ...
            functions.Add("and2", new TemplateFunction(FuncAnd2));
        ...
        object FuncAnd2(object[] args)
        {
            if (!CheckArgCount(3, "add2", args))
                return null;

            if (args[0] is bool && args[1] is bool && args[2] is bool)
                return (bool)args[0] && (bool)args[1] && (bool)args[2];
            else
                return false;
        }
        ...
        object FuncNoHTML(object[] args)
        {
            ...
            return JumboECMS.Utils.Strings.NoHTML(args[0].ToString());
        }
        object FuncCutString(object[] args)
        {
            if (!CheckArgCount(2, "cutstring", args))
                return null;
            return JumboECMS.Utils.Strings.CutString(args[0].ToString(), Convert.ToInt32(args[1].ToString()));
        }
        object FuncHTMLEncode(object[] args)
        {
            if (!CheckArgCount(1, "htmlencode", args))
                return null;
            return JumboECMS.Utils.Strings.HtmlEncode(args[0].ToString());
        }
        object FuncHTMLDecode(object[] args)
        {
            if (!CheckArgCount(1, "htmldecode", args))
                return null;
            return JumboECMS.Utils.Strings.HtmlDecode(args[0].ToString());
        }
        #endregion
        ...
        protected object EvalBinaryExpression(BinaryExpression exp)
        {
            switch (exp.Operator)
            {
                case JumboECMS.TEngine.Parser.TokenKind.OpOr:
                    {
                ...
                case JumboECMS.TEngine.Parser.TokenKind.OpAnd:
                    {
                        object lhsValue = EvalExpression(exp.Lhs);
                        if (!Util.ToBool(lhsValue))
                            return false;
                ...
                case JumboECMS.TEngine.Parser.TokenKind.OpIs:
                    {
                        object lhsValue = EvalExpression(exp.Lhs);
                        object rhsValue = EvalExpression(exp.Rhs);
                ...
                case JumboECMS.TEngine.Parser.TokenKind.OpIsNot:
                    {
                        object lhsValue = EvalExpression(exp.Lhs);
                        object rhsValue = EvalExpression(exp.Rhs);
                ...
                case JumboECMS.TEngine.Parser.TokenKind.OpGt:
                    {
                        object lhsValue = EvalExpression(exp.Lhs);
                        object rhsValue = EvalExpression(exp.Rhs);
                ...
                case JumboECMS.TEngine.Parser.TokenKind.OpLt:
                    {
                        object lhsValue = EvalExpression(exp.Lhs);
                        object rhsValue = EvalExpression(exp.Rhs);
                ...
                case JumboECMS.TEngine.Parser.TokenKind.OpGte:
                    {
                        object lhsValue = EvalExpression(exp.Lhs);
                        object rhsValue = EvalExpression(exp.Rhs);
                ...
                case JumboECMS.TEngine.Parser.TokenKind.OpLte:
                    {
                        object lhsValue = EvalExpression(exp.Lhs);
                        object rhsValue = EvalExpression(exp.Rhs);
