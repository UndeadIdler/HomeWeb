<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="category_edit1.aspx.cs" Inherits="JumboECMS.WebFile.Admin.category_edit1" %>
...
$(document).ready(function(){
	...
	$("#txtFolder").formValidator({tipid:"tipFolder",onshow:"只支持英文字母、数字和短线",onfocus:"一旦保存将无法修改",oncorrect:"OK"}).RegexValidator({regexp:"^\([0-9a-zA-Z\-]+)$",onerror:"只支持英文字母、数字和短线"});
	$("#txtAliasPage").formValidator({empty:true,tipid:"tipAliasPage",onshow:"自定义url，不输入即为默认。如/aa/bb/cc.html",onfocus:"自定义url，不输入即为默认。如/aa/bb/cc.html",oncorrect:"OK"}).RegexValidator({regexp:"^\(/[_\-a-zA-Z0-9\.]+(/[_\-a-zA-Z0-9\.]+)*\.(aspx|htm(l)?|shtm(l)?))$",onerror:"格式错误"});
...
<form id="form1" runat="server" onsubmit="return CheckFormSubmit()">
	<table class="formtable mrg10T">
		...
		<tr>
			<th> 目录名称</th>
			<td><asp:TextBox ID="txtFolder" runat="server" MaxLength="20" Width="160px" CssClass="ipt"></asp:TextBox> 
                <input ID="btnPinYin" runat="server" type="button" value="使用首字母" class="btnsubmit"  OnClick="ajaxChinese2Pinyin($('#txtTitle').val(),0);" />
                <input ID="btnPinYin2" runat="server" type="button" value="使用全拼" class="btnsubmit"  OnClick="ajaxChinese2Pinyin($('#txtTitle').val(),1);" />
		...
		<tr>
			<th> 默认模型 </th>
			<td><asp:DropDownList ID="ddlModuleType" runat="server"> </asp:DropDownList>
				(在前台能取到的循环调用数据) </td>
		</tr>
		<tr>
			<th> 模板 </th>
			<td><asp:DropDownList ID="ddlTemplateId" runat="server"> </asp:DropDownList>
			(如果不选择模板，就必须在自定义url中输入链接地址) </td>
		</tr>
		<tr>
			<th> 自定义url </th>
